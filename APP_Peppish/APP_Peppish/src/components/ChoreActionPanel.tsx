...
export const ChoreActionPanel = ({
  ...
  const approveMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      queryClient.setQueryData(["chores"], (old: any[] | undefined) => {
        ...
        return old.map((c) =>
          c.id === chore.id
            ? { ...c, status: "approved", updatedAt: new Date().toISOString() }
            : c,
        );
      });
      return { previous };
  ...
  const canComplete =
    chore.uiStatus === "Pending" && chore.assignedTo === user?.id;
  const canApprove = chore.uiStatus === "Completed" && user?.role === "adult";
  const canEditOrDelete =
    allowAdminActions &&
    (user?.role === "adult" || user?.id === chore.createdBy);
  const canPick =
    allowPicking &&
    user?.role === "child" &&
    (chore.assignedTo === undefined || chore.assignedTo === null);
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      const previous = queryClient.getQueryData<any[]>(["chores"]);
      queryClient.setQueryData(["chores"], (old: any[] | undefined) =>
        old ? old.filter((c) => c.id !== chore.id) : old,
      );
      return { previous };
    },
    onSuccess: () => {
    ...
    onError: (err, _vars, context: any) => {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to pick and complete chore",
      );
    },
    onSettled: () => queryClient.invalidateQueries({ queryKey: ["chores"] }),
  });
  ...
  const scheduleMutation = useMutation({
    mutationFn: () =>
      choreService.scheduleChore(chore.id, user!.id, scheduleDate),
    onMutate: async () => {
      await queryClient.cancelQueries({ queryKey: ["chores"] });
      const previous = queryClient.getQueryData<any[]>(["chores"]);
      ...
      <div className="action-buttons">
        ...
        {allowAdminActions && user?.role === "adult" && (
          <div className="assign-section">
            ...
            <select
              value={assignTo || ""}
              onChange={(e) =>
                setAssignTo(e.target.value === "" ? undefined : e.target.value)
              }
            >
        ...
        {canPick && (
          <div className="pick-section">
            ...
            <button
              className="btn-pick-complete"
              onClick={() => pickAndComplete.mutate()}
              disabled={pickAndComplete.isPending}
            >
              {pickAndComplete.isPending
                ? "Working..."
                : "Pick & Mark Completed"}
            </button>
            <button
        ...
        {canEditOrDelete && (
          <div className="admin-actions">
            ...
            <button
              ...
              onClick={() => {
                if (
                  window.confirm("Are you sure you want to delete this chore?")
                ) {
