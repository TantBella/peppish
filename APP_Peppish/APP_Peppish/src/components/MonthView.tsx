...
export const MonthView = ({ chores, setSelectedDate, setViewMode }: Props) => {
 ...
 return (
   <>
     <div>
       <h2 className="month-title">{now.toLocaleDateString('sv-SE', { month: 'long', year: 'numeric' })}</h2>
     </div>
       ...
       {Array.from({ length: totalCells }, (_, i) => {
         ...
         return (
           <div key={key} className={`month-day ${isToday ? 'today' : ''}`}>
             <div className="day-circle" onClick={() => handleDayClick(date)}>{dayNumber}</div>
