    ...
    ILogger<AuthService> logger) : IAuthService
{
  ...
  private string GenerateJwtToken(ApplicationUser user, string role)
  {
    ...
    if (string.IsNullOrWhiteSpace(jwtKey))
      throw new InvalidOperationException("Jwt:Key saknas i AuthService");

    if (string.IsNullOrWhiteSpace(jwtIssuer))
      throw new InvalidOperationException("Jwt:Issuer saknas i AuthService");

    if (string.IsNullOrWhiteSpace(jwtAudience))
      throw new InvalidOperationException("Jwt:Audience saknas i AuthService");
