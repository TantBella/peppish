    ...
    INotificationService notificationService) : IChoreInstanceService
{
  ...
  public async Task<ChoreInstance> CompleteAsync(Guid id, CancellationToken cancellationToken = default)
  {
    ...
    try
    {
      var payload = System.Text.Json.JsonSerializer.Serialize(new { instanceId = instance.Id, completedBy = userId, completedAt = instance.CompletedAt });
      await notificationService.CreateNotificationAsync(new API_Peppish.DTOs.CreateNotificationRequest { UserId = assignment.AssignedToUserId, Type = "chore_completed", Payload = payload, HouseholdId = householdId });

      var adults = (await userManager.GetUsersInRoleAsync("Adult")).Where(u => u.HouseholdId == householdId);
      foreach (var adult in adults)
      {
        await notificationService.CreateNotificationAsync(new API_Peppish.DTOs.CreateNotificationRequest { UserId = adult.Id, Type = "chore_needs_approval", Payload = payload, HouseholdId = householdId });
      }
    }
